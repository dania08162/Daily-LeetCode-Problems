...
**Day 1** - Two Sum Problem (Python)
  - # Intuition
        The problem is basically checking which indexes can add up to the target. My first thought was to use a for-loop to go through each index and check which one can add to the target; if the number does not work, move on to the next one.
